...
const FacultyAssignment = () => {
  const { facultyAssignments, faculty, courses, sections, fetchFacultyAssignments, fetchFaculty, fetchCourses, fetchSections, addFacultyAssignment, updateFacultyAssignment, deleteFacultyAssignment } = useApp();
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all([fetchFacultyAssignments(), fetchFaculty(), fetchCourses(), fetchSections()])
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (selectedAssignment) {
      await updateFacultyAssignment(selectedAssignment.id, formData);
    } else {
    ...
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={handleAdd}>
          <PlusIcon size={15} /> Assign Faculty
        </button>
      </div>
