...
const Syllabus = () => {
  const { syllabus, courses, fetchSyllabus, fetchCourses, addSyllabus, updateSyllabus, deleteSyllabus } = useApp();
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all([fetchSyllabus(), fetchCourses()]).finally(() => setLoading(false));
  }, []);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (selectedSyllabus) {
      await updateSyllabus(selectedSyllabus.id, formData);
    } else {
        ...
        footer={
          <>
            ...
            <button className="btn btn-danger" onClick={async () => {
              if (selectedSyllabus) await deleteSyllabus(selectedSyllabus.id);
